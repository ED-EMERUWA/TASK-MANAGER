...
-- -----------------------------------------------------
-- Table `user_management_db`.`CompletionCriteria`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `user_management_db`.`CompletionCriteria` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `fk_CriteriaTask`
    FOREIGN KEY (`task_id`)
    REFERENCES `user_management_db`.`Task` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
)
ENGINE = InnoDB;
